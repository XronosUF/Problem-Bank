...
# Compute Answer
Ans = integral(f,x).simplify_full()
...
\begin{problem}
    Now, using the previous steps, we can calculate our answer!
        \[
        \int \sage{h(x)} dx = \answer{\sage{Ans(x)}}
        \]
\end{problem}
\end{problem}
\end{problem}
...
%%%%%%%%%%%%%%%%%%%%%%
%%\tagged{Ans@Long, Type@Compute, Topic@Integral, Sub@PartialFractions, Func@Rational, Func@Poly, File@0107}{
\begin{sagesilent}
a = NonZeroInt(-5,5)
b = RandInt(1,10)
c = RandInt(-3,3)
