...
\end{multipleChoice}
\begin{problem}
What test tells you this series Converges?

\begin{multipleChoice}
\choice{p-test with a p-value of $\sage{-b}$}
\choice[correct]{p-test with a p-value of $\sage{b}$}
\choice{Direct Comparison Test}
...
\begin{problem}
What test tells you this series Diverges?
...
\begin{multipleChoice}
\choice{p-test with a p-value of $\sage{-p+(1/q)}$}
\choice[correct]{p-test with a p-value of $\sage{p+(1/q)}$}
\choice{p-test with a p-value of $\sage{-p-(1/q)}$}
\choice{p-test with a p-value of $\sage{p-(1/q)}$}
\end{multipleChoice}
...
fCoeff=a^n*exp(a*c)/factorial(n)
TAY=fCoeff*(x-c)^n
...
#Define general taylor term
fCoeff=((-1)^n*a*factorial((p+n-1))*c^(-p-n))/(factorial(n)*factorial((p-1)))
TAY=fCoeff*(x-c)^n
...
#Define the general original function
f=(-1)^n*b^(2*n+1)*x^(p*(2*n+1))/(factorial(2*n+1))
...
#Define the general original function
f=(-1)^n*b^(2*n)*x^(p*(2*n))/(factorial(2*n))
