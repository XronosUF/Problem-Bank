...
\[\lim_{x\to\sage{b}}\dfrac{\sage{fNum}}{\sage{fDen}}=\answer[format=string]{DNE}\]
\end{problem}}%}
%%%%%%%%%%%%%%%%%%%%%
...
\[
\lim\limits_{x\to\sage{Inf}}\frac{\sage{fNum}}{\sage{fDen}}=\answer{\sage{Ans}}
\]
...
\[f(x)=\left\lbrace\begin{array}{ll}\sage{Fone}& x<\sage{a}\\[5pt] 
...
\begin{sagesilent}
# This problem generates a terrible looking function, that happens to be continuous at the desired limit target; to teach people that L'Hsopital's rule shouldn't be a reflex.


# Everything in the initial creation needs to avoid crazy large, or zero value, to make sure end result is valid and reasonable for students.

# initialize functions
ftop = x
fbot = x
# initialize target to force the while loop to trigger.
target = 0 

while ftop(target) == 0 or fbot(target) == 0 or ftop(target) > 1000 or fbot(target) > 1000:
    ## parameters of the master functions:
    a = RandInt(-5,5)
    aPlus = RandInt(1,10)
    aPlus2 = RandInt(aPlus,15)
    aAlt = NonZeroInt(-5,5)
    b = NonZeroInt(-5,5,[a])
    c = RandInt(-5,5)
    
    # Master function vector
    funcvec=[a*x^2+b*x+c, aAlt*(x-a)^(1/3)+c, aAlt*exp(x+a)+c, aAlt*log(x+aPlus)+c, aAlt*(x+aPlus2)^(1/2)+c]
    
    pick1 = 3#RandInt(0,4)# Pick numerator function
    ftop = funcvec[pick1]
    
    pick2 = 4#NonZeroInt(0,4,[pick1])# Pick a different denominator Function
    fbot = funcvec[pick2]
    
    ## Some parameters depend on the type of random function we selected.
    # Logs and square roots don't like negatives.
    if pick1 > 2 or pick2 > 2:
      target = RandInt(-aPlus + 1, 8)
    else:
      target = RandInt(-8, 8)

# Now we have a reasonable answer, so we just need to clean up.
...
$\lim\limits_{x \rightarrow \sage{target}} \sage{fFinal(x)} = \answer{\sage{ans}}$
