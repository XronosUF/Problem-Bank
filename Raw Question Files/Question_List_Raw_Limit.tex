...
\[\lim_{x\to \sage{b}^+}\sage{F}=\answer{\sage{Ans}}\]
...
1+(\sage{f})^2 & , & x\leq \sage{a}\\
\sage{b}-x & , & \sage{a}<x\leq\sage{b}\\
\sage{k} & , & x>\sage{b}
\end{array}\right.$.  
...
The limit as $x\to0$ of $f(x)=\sage{F}$ is $0$.  What is the reason why this is true?

\input{2311_Concept_Limit_0008.HELP.tex}

\begin{multipleChoice}
\choice{The statement is in fact false: $\lim\limits_{x\to0}\sage{F}\neq0$.}
